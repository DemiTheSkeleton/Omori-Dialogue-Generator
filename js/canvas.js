const normalFont = new FontFace(
  "OMORI_MAIN",
  "url(./css/fonts/OMORI_GAME.ttf)"
);
const disturbedFont = new FontFace(
  "OMORI_DISTURBED",
  "url(./css/fonts/OMORI_GAME2.ttf)"
);
...
function renderCanvas(idFrame, idDownload) {
  ...
  function insertDialogue(context, unsplitText) {
    ...
    for (let lineNo = 0; lineNo < splitText.length; lineNo++) {
      ...
      if (lineNo >= 3) {
        alert(
          "Only a maximum of three lines can fit. The fourth line and so has not been rendered."
        );
        break;
      }
